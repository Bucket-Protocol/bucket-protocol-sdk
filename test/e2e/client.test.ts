...
describe('Interacting with Bucket Client on mainnet', () => {
  describe('Oracle', () => {
    it('getAllOraclePrices returns positive numbers for all oracle coin types', async () => {
  ...
  describe('Config & metadata', () => {
    it('usdbCoinType returns USDB metadata (6 decimals)', async () => {
    ...
    it('getFlashMintInfo returns feeRate and partner config', async () => {
  ...
  describe('Positions & accounts', () => {
    it('getAccountPositions length <= getUserPositions length', async () => {
  ...
  describe('Rewards', () => {
    it('buildClaimBorrowRewardsTransaction dry run succeeds and has reward balance changes', async () => {
  ...
  describe('PSM', () => {
    it('buildPSMSwapInTransaction: 1 USDC -> USDB, dry run and balance delta', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
      const amount = 1 * 10 ** 6;
    ...
    it('buildPSMSwapOutTransaction: 1 USDB -> USDC, dry run succeeds', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
      const amount = 1 * 10 ** 6;
  ...
  describe('Saving', () => {
    it('psmSwapIn then deposit to saving pool', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
    ...
    it('deposit to saving pool', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
      const amount = 0.1 * 10 ** 6;
      const usdbCoin = coinWithBalance({ type: usdbCoinType, balance: amount });
    ...
    it('withdraw from saving pool', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
      const amount = 0.1 * 10 ** 6;
    ...
    it('claim from saving pool', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
    ...
    it('deposit/withdraw zero to saving pool', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
  ...
  describe('Vaults', () => {
    ...
    it('getAllVaultObjects: one entry per collateral type, all with required VaultInfo fields', async () => {
    ...
    it('getAllVaultObjects: at least one SCALLOP vault has non-empty rewardRate', async () => {
      const allVaults = await bucketClient.getAllVaultObjects();
    ...
    it('getVaultObjectInfo returns config for known collateral (SUI)', () => {
    ...
    it('getBorrowRewardFlowRate returns record for SUI vault', async () => {
    ...
    it('SUI vault has minCollateralRatio in reasonable range (e.g. 110%–200%)', async () => {
      const allVaults = await bucketClient.getAllVaultObjects();
  ...
  describe('CDP', () => {
    it('buildManagePositionTransaction: fixed deposit/borrow, event shape', async () => {
    ...
    it('buildManagePositionTransaction: borrow 1 USDB at CR 110% using live SUI price', async () => {
    ...
    it('buildClosePositionTransaction dry run succeeds when account has SUI position', async () => {
  ...
  describe('Flash', () => {
    it('flashMint 1000 USDB then flashBurn', async () => {
      const tx = new Transaction();
      tx.setSender(testAccount);
