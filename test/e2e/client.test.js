...
describe('Interacting with Bucket Client on mainnet', () => {
    ...
    it('test buildManagePositionTransaction()', async () => {
        ...
        if (!positionUpdatedEvent)
            return;
        const positionUpdatedEventData = positionUpdatedEvent.parsedJson;
    ...
    it('test buildClaimBorrowRewardsTransaction()', async () => {
        ...
        bucketClient.getAllCollateralTypes().map((coinType) => {
            ...
            if (rewards)
                tx.transferObjects(rewards, testAccount);
        });
    ...
    it('test getAllVaultObjects()', async () => {
        ...
        Object.entries(allVaults)
            ...
            .map(([, vault]) => {
            expect(vault.rewardRate['0x7016aae72cfc67f2fadf55769c0a7dd54291a583b63051a5ed71081cce836ac6::sca::SCA']).toBeDefined();
        });
    });
    it('test buildMigrateSBuckToSUsdbTransaction()', async () => {
        ...
        const res = await suiClient.getOwnedObjects({
            owner: account,
            filter: {
                StructType: '0x75b23bde4de9aca930d8c1f1780aa65ee777d8b33c3045b053a178b452222e82::fountain_core::StakeProof<0x1798f84ee72176114ddbf5525a6d964c5f8ea1b3738d08d50d0d3de4cf584884::sbuck::SBUCK, 0x2::sui::SUI>',
            },
        });
        const proofIds = res.data.map((d) => d.data?.objectId);
