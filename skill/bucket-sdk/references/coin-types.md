...
> **Note**: Coin types may change as the protocol adds or removes support. Call `await client.getAllCollateralTypes()`, `await client.getAllOracleCoinTypes()`, or inspect `await client.getConfig()` for the live list.
...
Get it programmatically with `await client.getUsdbCoinType()`.
