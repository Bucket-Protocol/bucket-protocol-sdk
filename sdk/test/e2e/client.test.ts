...
describe('Interacting with Bucket Client on testnet', () => {
  ...
  it('test getter functions for SavingPools on testnet', async () => {
    const network = 'testnet';
    const suiClient = new SuiClient({ url: getFullnodeUrl(network) });
    const bucketClient = new BucketV2Client({ suiClient, network });
    ...
    expect(lpBalance > 0);
    expect(lpValue > lpBalance && lpValue > 0);
    Object.values(realtimeRewards).forEach((v) => expect(v > 0));
  });
  ...
  it('test psmSwapOut() on testnet', async () => {
    ...
    const collateralCoin = await bucketClient.buildPSMSwapOutTransaction(tx, {
      coinType: COIN_TYPES.USDC,
      usdbCoin,
    });
