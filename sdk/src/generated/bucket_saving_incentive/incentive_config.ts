...
export function packageVersion(options: PackageVersionOptions = {}) {
  const packageAddress =
    options.package ?? "@local-pkg/bucket_saving_incentive";
...
export interface AssertValidPackageVersionOptions {
  package?: string;
  arguments:
    | AssertValidPackageVersionArguments
    | [config: RawTransactionArgument<string>];
}
/** Public Funs */
export function assertValidPackageVersion(
  options: AssertValidPackageVersionOptions,
) {
  const packageAddress =
    options.package ?? "@local-pkg/bucket_saving_incentive";
  const argumentsTypes = [
